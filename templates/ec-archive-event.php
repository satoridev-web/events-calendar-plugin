...
$query_args  = $_GET; // used to preserve query vars in view-toggle buttons
?>
	...
	<div class="ec-event-filters-wrapper">
		...
		<form class="ec-event-filters" method="get" action="<?php echo esc_url($base_url); ?>">
			...
			<div class="ec-filter-search">
				...
				<label for="ec-search" class="screen-reader-text">
					<?php esc_html_e('Search events', 'events-calendar-plugin'); ?>
				</label>
				<input
					type="search"
					id="ec-search"
			...
			<?php
			// Preserve any other custom query vars
		...
		<!-- View Toggle Buttons -->
		<div class="ec-view-toggle" role="group" aria-label="<?php esc_attr_e('Toggle between grid and list views', 'events-calendar-plugin'); ?>">
			...
			<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
					<div class="ec-list-card">
						...
						<div class="ec-list-content">
							...
							<div class="ec-list-excerpt"><?php echo esc_html(\Satori_EC\ec_get_clean_excerpt(get_the_ID(), 30)); ?></div>
						</div>
					</div>
				<?php endwhile;
			else : ?>
			...
			<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
					<div class="ec-archive-card">
						...
						<div class="ec-archive-card-content">
							<h2 class="ec-archive-card-title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h2>
							<div class="ec-event-excerpt"><?php echo esc_html(\Satori_EC\ec_get_clean_excerpt(get_the_ID(), 30)); ?></div>
							...
							<a href="<?php the_permalink(); ?>" class="ec-event-read-more">
								<?php esc_html_e('Read More →', 'events-calendar-plugin'); ?>
							</a>
						</div>
					</div>
				<?php endwhile;
